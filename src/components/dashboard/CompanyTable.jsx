...
const CompanyTable = ({ companies }) => {
    ...
    const handleApprove = async (id, name) => {
        ...
        if (result.modifiedCount) {
            ...
            toast.success(`Approved company name: ${name}`)
        }
                                ...
                                return (
                                    <Table.Row key={companyId} className="border-b border-neutral-800/50 hover:bg-neutral-900/30 transition-colors">
                                        ...
                                        <Table.Cell className="py-4 align-middle text-neutral-400">
                                            {company?.recruiterEmail || `recruiter@${company.companyName.toLowerCase().replace(/\s+/g, '')}.com`}
                                        </Table.Cell>
                                            ...
                                            <div className="flex justify-end gap-2">
                                                {company.status?.toLowerCase() !== 'approved' && (
                                                    <Button
                                                        size="sm"
                                                        variant="light"
                                                        onClick={() => handleApprove(companyId, company.companyName)}
                                                ...
                                                {company.status?.toLowerCase() !== 'rejected' && (
                                                    <Button
                                                        size="sm"
                                                        variant="light"
                                                        onClick={() => handleReject(companyId, company.companyName)}
